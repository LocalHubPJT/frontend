...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(posts.value.length / size)),
)

const paged = computed(() =>
  posts.value.slice((page.value - 1) * size, page.value * size),
)
...
async function load() {
  ...
  try {
    posts.value = await service.posts(keyword.value)
    page.value = 1
  } catch (loadError) {
...
<template>
  <section class="page-heading board-heading">
    <div>
      ...
      <h1>대전 여행 커뮤니티</h1>
      <p>로그인 없이 익명으로 여행 경험과 지역 정보를 자유롭게 공유해 보세요.</p>
    </div>
    <router-link class="button" to="/board/write">글쓰기</router-link>
  </section>
  ...
  <div v-else class="board-card">
    ...
    <p v-if="loading" class="empty-state">게시글을 불러오는 중입니다…</p>
  ...
  <div v-if="totalPages > 1" class="pagination">
    ...
    <button @click="page = Math.min(totalPages, page + 1)">›</button>
  </div>
</template>
